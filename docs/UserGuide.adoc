...
== Deleting person(s) by name: `deletebyname`

Deletes all persons matching the names entered from the address book. Irreversible. +
Format: `deletebyname NAME`

****
Deletes all persons matching the same `NAME`.
****
...
* `list` +
`deletebyname John` +
Deletes the person(s) containing the name 'John' in the address book.

* `find John Doe` +
`deletebyname John Doe` +
Deletes person(s) with the name John or Doe or John Doe.
